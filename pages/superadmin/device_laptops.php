...
if (! empty($ep_filter)) {
    // endpoint_security_id is stored as a JSON array of ids on the laptops row,
    // e.g. "[2,5]" — so we check membership with JSON_CONTAINS rather than IN(),
    // guarding against NULL/empty/malformed JSON on legacy rows. A device shows
    // up if it has ANY ONE of the selected endpoint security options.
    ...
    foreach ($ep_filter as $v) {
        ...
        $baseParams[] = (string)$v;             // matches integer JSON: [3,7]
        $baseParams[] = json_encode((string)$v); // matches string JSON: ["3","7"]
        $baseTypes  .= 'ss';
    }
    $baseWhere[] = '(' . implode(' OR ', $conditions) . ')';
}
