...
// ── Build the same WHERE clause as the main page ──────────────────────────────
...
$office_filter   = is_array($office_raw)   ? array_filter(array_map('trim', $office_raw))   : [];
...
$stmt = $conn->prepare("
    ...
    $baseJoin $whereSQL ORDER BY d.device_name ASC
");
...
// ── Output as Excel (tab-separated, .xls compatible) via HTML table ───────────
// Using HTML-table-as-XLS trick — works in all PHP versions without extra libs.
