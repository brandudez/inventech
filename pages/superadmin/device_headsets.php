...
$activeWhere   = $where;
$activeWhere[] = "h.is_active = 1";
$activeSQL     = "WHERE " . implode(" AND ", $activeWhere);

$inactiveWhere   = $where;
$inactiveWhere[] = "h.is_active = 0";
...
========================= */
$stmt = $conn->prepare("
    SELECT h.*, CONCAT(COALESCE(r.rank,''),' ',per.first_name,' ',per.middle_name,' ',per.last_name) AS fullname,
           d.division AS division_name
    $baseJoin $whereSQL ORDER BY h.id DESC LIMIT ?,?
");
$fp = $params;
$ft = $types . 'ii';
$fp[] = $offset;
    ...
    <div class="top-bar">
        <div class="search-container">
            <form class="search-form" method="GET" action="device_headsets.php">
                ...
                <!-- EXPORT BUTTON -->
                <a href="export_headsets.php?<?= htmlspecialchars($exportParams) ?>" class="btn add-laptop-btn" title="Export current filtered data to Excel">
        ...
        <div class="table-container">
            <table class="users-table">
                ...
                <tbody>
                        ...
                        <?php while ($row = $result->fetch_assoc()): ?>
                            ...
                            <div class="modal fade" id="viewHsModal<?= $row['id'] ?>" tabindex="-1" aria-hidden="true">
                                <div class="modal-dialog modal-lg modal-dialog-scrollable modal-dialog-centered">
                                    <div class="modal-content">
                                        <div class="modal-header text-white" style="background-color:#0d6ea8;">
                                            <h5 class="modal-title"><i class="bi bi-headset me-2"></i>Headset Details — <?= htmlspecialchars($row['brand'] . ' ' . $row['model']) ?></h5>
                                            <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                                        </div>
                                        <div class="modal-body">
                                            <div class="row g-3">
                                                                ...
                                                                foreach ($allPersonnel as $p): $fn = trim(($p['rank'] ?? '') . ' ' . ($p['last_name'] ?? '') . ' ' . ($p['first_name'] ?? '') . ' ' . ($p['middle_name'] ?? '')); ?>
                                                                    <div class="form-check">
                                                                        <input class="form-check-input" type="checkbox" name="previous_handlers_id[]"
                                                                            value="<?= $p['id'] ?>" id="editHsPh<?= $row['id'] ?>_<?= $p['id'] ?>"
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="11" class="text-center">No headsets found.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
    ...
    <script>
        ...
        <?php if (isset($_GET['edit'])): ?>
            document.addEventListener("DOMContentLoaded", function() {
                let modal = document.getElementById("editHsModal<?= (int)$_GET['edit'] ?>");
