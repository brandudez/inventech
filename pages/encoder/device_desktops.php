...
// ── Build base WHERE + params (always locked to encoder's division) ───────
$baseWhere  = ["d.division_id = ?"];
$baseParams = [$encoderDivisionId];
$baseTypes  = 'i';
...
if ($acq_filter === 'lt5') {
    ...
    $baseWhere[] = "(d.acquisition_date IS NULL OR d.acquisition_date = '0000-00-00' OR d.acquisition_date = '')";
}
...
$st->bind_param($baseTypes, ...$baseParams);
$st->execute();
$totalDevices = $st->get_result()->fetch_assoc()['total'] ?? 0;
$totalPages   = (int)ceil($totalDevices / $limit);

// ── Stat-box counts ───────────────────────────────────────────────────────
$activeWhere   = array_merge($baseWhere, ["d.is_active = 1"]);
$inactiveWhere = array_merge($baseWhere, ["d.is_active = 0"]);

$sa = $conn->prepare("SELECT COUNT(*) AS total $baseJoin WHERE " . implode(" AND ", $activeWhere));
$sa->bind_param($baseTypes, ...$baseParams);
$sa->execute();
$activeDevices = $sa->get_result()->fetch_assoc()['total'] ?? 0;

$si = $conn->prepare("SELECT COUNT(*) AS total $baseJoin WHERE " . implode(" AND ", $inactiveWhere));
$si->bind_param($baseTypes, ...$baseParams);
$si->execute();
...
");
$fp  = array_merge($baseParams, [$offset, $limit]);
$ft  = $baseTypes . 'ii';
$stmt->bind_param($ft, ...$fp);
$stmt->execute();
$result = $stmt->get_result();

// ── Pre-fetch data for Add/Edit modals (scoped to encoder's division) ─────
$addPersonnelRows = [];
$pq = $conn->prepare("
...
// ── Encoder's division info (fixed — not a dropdown) ─────────────────────
$divRow = $conn->prepare("SELECT id, division FROM divisions WHERE id = ?");
$divRow->bind_param("i", $encoderDivisionId);
$divRow->execute();
$encoderDivision = $divRow->get_result()->fetch_assoc();
...
<body>
            ...
            <div class="modal-content custom-modal">
                ...
                <div class="modal-body">
                    <form action="add_desktop.php" method="POST" id="addDesktopForm">
                        ...
                        <!-- Division is fixed to encoder's division — hidden + display only -->
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-4">
                                <label class="form-label">CPU Generation</label>
                                <input type="number" class="form-control" name="cpu_generation" min="1" placeholder="e.g. 11">
                            </div>
                            <div class="col-md-4">
                            ...
                            <input type="hidden" name="no_of_user_accounts" id="addDtAccountCount">

                            <div class="col-md-12">
                                <label class="form-label">User Account Type</label>
                                <div id="addDtAccountContainer">
                                    <div class="account-row">
                                        ...
                                        <button type="button" class="btn btn-success btn-icon" onclick="addAccountRow('addDtAccountContainer')">
                                            <i class="bi bi-plus-lg"></i>
                                        </button>
                                    </div>
                                </div>
                                <input type="hidden" name="user_account_type" id="addDtAccountJson">
                            </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="dropdown w-100">
                                    ...
                                    <div class="dropdown-menu w-100 p-2" style="max-height:250px;overflow-y:auto;">
                                        <?php foreach ($addPersonnelRows as $h):
                                    ...
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            <form action="edit_desktops.php" method="POST"
                                                id="editDtForm<?= $row['id'] ?>"
                                                onsubmit="buildAccountJson('editDtAccountContainer<?= $row['id'] ?>', 'editDtAccountJson<?= $row['id'] ?>', 'editDtAccountCount<?= $row['id'] ?>')">
                                                ...
                                                <div class="row g-3">
                                                    ...
                                                    <div class="col-md-6">
                                                        ...
                                                        <select name="office_application" class="form-select">
                                                            <?php foreach ($officeAppsList as $app):
                                                                $isSelected = ($row['office_application'] ?? '') == $app || (trim($row['office_application'] ?? '') === '' && trim($app) === '-'); ?>
                                                                <option value="<?= htmlspecialchars($app) ?>" <?= $isSelected ? 'selected' : '' ?>><?= htmlspecialchars($app) ?></option>
                                                            <?php endforeach; ?>
                                                        </select>
                                                    </div>
                                                    <div class="col-md-4">
                                                    ...
                                                    <div class="col-md-4">
                                                        <label class="form-label">CPU Generation</label>
                                                        <input type="number" class="form-control" name="cpu_generation" min="1" placeholder="e.g. 11" value="<?= htmlspecialchars($row['cpu_generation'] ?? '') ?>">
                                                    </div>
                                                    <div class="col-md-4">
                                                    ...
                                                    <input type="hidden" name="no_of_user_accounts" id="editDtAccountCount<?= $row['id'] ?>">

                                                    <div class="col-md-12">
                                                        <label class="form-label">User Account Type</label>
                                                        <div id="editDtAccountContainer<?= $row['id'] ?>">
                                                            <?php
                                                                ...
                                                                <div class="account-row">
                                                                    ...
                                                                    <?php if ($isFirst): ?>
                                                                        <button type="button" class="btn btn-success btn-icon"
                                                                            onclick="addAccountRow('editDtAccountContainer<?= $row['id'] ?>')">
                                                                            <i class="bi bi-plus-lg"></i>
                                                                        </button>
                                                                    <?php else: ?>
                                                        ...
                                                        <input type="hidden" name="user_account_type" id="editDtAccountJson<?= $row['id'] ?>">
                                                    </div>
    ...
    <script>
        ...
        document.getElementById("addDesktopForm").addEventListener("submit", function(e) {
            ...
            buildAccountJson('addDtAccountContainer', 'addDtAccountJson', 'addDtAccountCount');
        });
