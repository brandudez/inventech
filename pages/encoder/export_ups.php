...
if (!empty($search)) {
    $where[] = "(
        u.brand LIKE ? OR u.model LIKE ? OR u.serial_no LIKE ? OR
...
while ($row = $result->fetch_assoc()) {
    $cells = [
        ...
        dashExport($row['capacity_va']         ?? ''),
        dashExport($row['capacity_watts']      ?? ''),
        dashExport($row['battery_type']        ?? ''),
        dashExport($row['backup_time']         ?? ''),
        dashExport($row['input_voltage']       ?? ''),
        dashExport($row['output_voltage']      ?? ''),
