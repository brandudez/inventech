...
function getCpuGenStatus($gen)
{
    if ($gen === null || $gen === '' || !is_numeric($gen)) {
        return ['label' => '-', 'badge' => 'secondary', 'display' => '-'];
    }
    $g = (int)$gen;
    if ($g <= 7) {
        return ['label' => 'End of Life',                    'badge' => 'danger',  'display' => $g . 'th Gen'];
    } elseif ($g <= 10) {
        return ['label' => 'Near End of Life',               'badge' => 'warning', 'display' => $g . 'th Gen'];
    } else {
        return ['label' => 'Recommended for continued use',  'badge' => 'success', 'display' => $g . 'th Gen'];
    }
}
...
if (! empty($search)) {
    $baseWhere[] = "(l.device_name LIKE ? OR CONCAT(p.first_name,' ',p.middle_name,' ',p.last_name) LIKE ? OR l.ip_address LIKE ? OR l.guid LIKE ? OR l.mac_address LIKE ?)";
    $sv = "%$search%";
    for ($i = 0; $i < 5; $i++) {
        $baseParams[] = $sv;
        $baseTypes .= 's';
...
if (! empty($os_filter)) {
    ...
    foreach ($os_filter as $v) {
        if ($v === '-') {
            $conditions[] = "(l.os IS NULL OR l.os = '' OR l.os = '-')";
        } else {
            $conditions[] = "l.os = ?";
            $baseParams[] = $v;
            $baseTypes .= 's';
...
if (! empty($office_filter)) {
    ...
    foreach ($office_filter as $v) {
        if ($v === '-') {
            $conditions[] = "(l.office_application IS NULL OR l.office_application = '' OR l.office_application = '-')";
        } else {
...
if ($active_filter !== '') {
    $baseWhere[] = "l.is_active = ?";
    $baseParams[] = $active_filter;
    $baseTypes .= 'i';
}
if ($acq_filter === 'lt5') {
    $baseWhere[] = "l.acquisition_date IS NOT NULL AND l.acquisition_date != '0000-00-00' AND l.acquisition_date >= DATE_SUB(CURDATE(), INTERVAL 5 YEAR)";
} elseif ($acq_filter === 'gt5') {
    $baseWhere[] = "l.acquisition_date IS NOT NULL AND l.acquisition_date != '0000-00-00' AND l.acquisition_date < DATE_SUB(CURDATE(), INTERVAL 5 YEAR)";
} elseif ($acq_filter === 'none') {
    $baseWhere[] = "(l.acquisition_date IS NULL OR l.acquisition_date = '' OR l.acquisition_date = '0000-00-00')";
}
if ($cpu_gen_filter === 'eol') {
    $baseWhere[] = "(l.cpu_generation IS NOT NULL AND l.cpu_generation != '' AND CAST(l.cpu_generation AS UNSIGNED) <= 7)";
} elseif ($cpu_gen_filter === 'near_eol') {
    $baseWhere[] = "(l.cpu_generation IS NOT NULL AND l.cpu_generation != '' AND CAST(l.cpu_generation AS UNSIGNED) BETWEEN 8 AND 10)";
} elseif ($cpu_gen_filter === 'good') {
    $baseWhere[] = "(l.cpu_generation IS NOT NULL AND l.cpu_generation != '' AND CAST(l.cpu_generation AS UNSIGNED) >= 11)";
} elseif ($cpu_gen_filter === 'none') {
    $baseWhere[] = "(l.cpu_generation IS NULL OR l.cpu_generation = '')";
}
...
<head>
    ...
    <style>
        ...
        .cpu-gen-dropdown .dropdown-menu {
...
<body>
    ...
    <div class="top-bar">
        <div class="search-container">
            <form class="search-form" method="GET" action="admin_device_laptops.php">
        ...
        <div class="right-side">
            <div class="filters">
                <form method="GET" action="admin_device_laptops.php" id="filterForm">
            ...
            <div class="dropdown cpu-gen-dropdown">
                <?php
                $cpuGenLabel = 'CPU Gen';
                if ($cpu_gen_filter === 'eol')          $cpuGenLabel = '≤7th Gen (End of Life)';
                elseif ($cpu_gen_filter === 'near_eol') $cpuGenLabel = '8–10th Gen (Near EOL)';
                ...
                <ul class="dropdown-menu p-3" style="min-width:max-content;white-space:nowrap;">
                    <li><a class="dropdown-item" href="<?= $cpuGenBase ?>">All</a></li>
                    <li><a class="dropdown-item" href="<?= $cpuGenBase ?>&filter_cpu_gen=eol">7th Gen &amp; below (End of Life)</a></li>
                    <li><a class="dropdown-item" href="<?= $cpuGenBase ?>&filter_cpu_gen=near_eol">8th – 10th Gen (Near End of Life)</a></li>
                    <li><a class="dropdown-item" href="<?= $cpuGenBase ?>&filter_cpu_gen=good">11th Gen &amp; above (Recommended)</a></li>
                    <li><a class="dropdown-item" href="<?= $cpuGenBase ?>&filter_cpu_gen=none">No CPU Gen Specified</a></li>
                </ul>
            </div>
        ...
        <div class="modal-dialog modal-xl modal-dialog-scrollable">
            <div class="modal-content custom-modal">
                ...
                <div class="modal-body">
                    <form action="admin_add_laptop.php" method="POST" id="addLaptopForm">
                        <div class="row g-3">
                            ...
                            <div class="col-md-4">
                                ...
                                <input type="text" class="form-control" name="os_license_key" placeholder="e.g. XXXXX-XXXXX-XXXXX">
                            </div>
                            <div class="col-md-6">
                            ...
                            <div class="col-md-3">
                                ...
                                <input type="text" class="form-control" name="office_license_key" placeholder="e.g. XXXXX-XXXXX-XXXXX">
                            </div>
                            <div class="col-md-12">
                            ...
                            <input type="hidden" name="no_of_user_accounts" id="addAccountCountL">

                            <div class="col-md-12">
                                <label class="form-label">User Account Type</label>
                                <div id="addAccountContainerL">
                                    <div class="account-row">
                                        ...
                                        <button type="button" class="btn btn-success btn-icon" onclick="addAccountRow('addAccountContainerL')">
                                            <i class="bi bi-plus-lg"></i>
                                        </button>
                                    </div>
                                </div>
                                <input type="hidden" name="user_account_type" id="addAccountJsonL">
                            </div>
                ...
                <tbody>
                    <?php if ($result->num_rows > 0): ?>
                        <?php while ($row = $result->fetch_assoc()):
                            $cpuSt = getCpuGenStatus($row['cpu_generation'] ?? null);
                        ?>
                            <tr class="clickable-row" data-active="<?= $row['is_active'] ? '1' : '0' ?>"
                                ...
                                <td>
                                    <?php if ($cpuSt['label'] !== '-'): ?>
                                        <?= htmlspecialchars($cpuSt['display']) ?>
                                        <span class="badge bg-<?= $cpuSt['badge'] ?> ms-1"><?= htmlspecialchars($cpuSt['label']) ?></span>
                                        <?php else: ?>-<?php endif; ?>
                                </td>
                                ...
                                <td onclick="event.stopPropagation();">
                                    <button class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#editLtModal<?= $row['id'] ?>">
                            ...
                            <div class="modal fade" id="viewLtModal<?= $row['id'] ?>" tabindex="-1" aria-hidden="true">
                                <div class="modal-dialog modal-xl modal-dialog-scrollable modal-dialog-centered">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-footer">
                                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                                            <button type="button" class="btn btn-primary" data-bs-dismiss="modal" data-edit-target="#editLtModal<?= $row['id'] ?>">
                            ...
                            <!-- ── EDIT MODAL ── -->
                            <div class="modal fade" id="editLtModal<?= $row['id'] ?>" tabindex="-1" aria-hidden="true">
                                <div class="modal-dialog modal-xl modal-dialog-scrollable modal-dialog-centered">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                        ...
                                        <div class="modal-body">
                                            <form action="admin_edit_laptops.php" method="POST"
                                                ...
                                                <div class="row g-3">
                                                    ...
                                                    <div class="col-md-4">
                                                        <label class="form-label">OS License Key</label>
                                                        <input type="text" class="form-control" name="os_license_key" value="<?= htmlspecialchars($row['os_license_key'] ?? '') ?>" placeholder="e.g. XXXXX-XXXXX-XXXXX">
                                                    </div>
                                                    <div class="col-md-6">
                                                        <label class="form-label">Office Application</label>
                                                        <select name="office_application" class="form-select">
                                                    ...
                                                    <!-- ++ OFFICE LICENSE KEY EDIT ++ -->
                                                    <div class="col-md-3">
                                                        <label class="form-label">Office License Key</label>
                                                        <input type="text" class="form-control" name="office_license_key" value="<?= htmlspecialchars($row['office_license_key'] ?? '') ?>" placeholder="e.g. XXXXX-XXXXX-XXXXX">
                                                    </div>
                                                    ...
                                                    <div class="col-md-12">
                                                        ...
                                                        <div class="row">
                                                            ...
                                                            while ($ep = mysqli_fetch_assoc($epQ)): ?>
                                                                <div class="col-md-4">
                                                                    <div class="form-check">
                                                                        <input class="form-check-input ep-checkbox" type="checkbox" name="endpoint_security[]" value="<?= $ep['id'] ?>" id="epLt<?= $row['id'] . '_' . $ep['id'] ?>" <?= in_array($ep['id'], $selectedEP) ? 'checked' : '' ?>>
                                                                        <label class="form-check-label" for="epLt<?= $row['id'] . '_' . $ep['id'] ?>"><?= htmlspecialchars($ep['antivirus']) ?></label>
                                                                    </div>
                                                                </div>
                                                            <?php endwhile; ?>
                                                        </div>
                                                        ...
                                                        <div class="dropdown w-100">
                                                            ...
                                                            <div class="dropdown-menu w-100 p-2" style="max-height:250px;overflow-y:auto;">
                                                                ...
                                                                while ($h = mysqli_fetch_assoc($hQ)): $fn = trim(($h['rank'] ?? '') . ' ' . ($h['last_name'] ?? '') . ' ' . ($h['first_name'] ?? '') . ' ' . ($h['middle_name'] ?? '')); ?>
                                                                    <div class="form-check">
                                                                        <input class="form-check-input" type="checkbox" name="previous_owners_id[]" value="<?= $h['id'] ?>" id="phLt<?= $row['id'] . '_' . $h['id'] ?>" <?= in_array($h['id'], $selH) ? 'checked' : '' ?>>
                                                                        <label class="form-check-label" for="phLt<?= $row['id'] . '_' . $h['id'] ?>"><?= htmlspecialchars($fn) ?></label>
                                                                    </div>
                                                                <?php endwhile; ?>
                                                            </div>
                                                        </div>
    ...
    <script>
        ...
        document.getElementById('addLaptopForm').addEventListener('submit', function(e) {
            ...
            buildAccountJson('addAccountContainerL', 'addAccountJsonL', 'addAccountCountL');
        });
