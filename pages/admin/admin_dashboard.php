...
$sql_users_div = "
    SELECT d.division,
           COUNT(CASE WHEN u.is_active = 1 AND u.role_id != 1 THEN 1 END) AS total,
           COUNT(CASE WHEN u.is_active = 1 AND u.role_id != 1 THEN 1 END) AS active,
           COUNT(CASE WHEN u.is_active = 0 AND u.role_id != 1 THEN 1 END) AS inactive
    FROM divisions d
...
/* ════════════════════════════════════════════
   ...
   ════════════════════════════════════════════ */
$device_tables = ['laptops', 'desktops', 'printers', 'cameras', 'headsets', 'switches', 'routers', 'firewalls', 'switchers', 'splitters', 'ups', 'others'];
$device_maps = [];
...
$dev_switchers = array_map(fn($l) => $device_maps['switchers'][$l] ?? 0, $div_labels);
$dev_splitters = array_map(fn($l) => $device_maps['splitters'][$l] ?? 0, $div_labels);
$dev_ups       = array_map(fn($l) => $device_maps['ups'][$l]       ?? 0, $div_labels);
$dev_others    = array_map(fn($l) => $device_maps['others'][$l]    ?? 0, $div_labels);
...
$total_switchers = array_sum($dev_switchers);
$total_splitters = array_sum($dev_splitters);
$total_ups       = array_sum($dev_ups);
$total_others    = array_sum($dev_others);
...
$cnt_switchers = countTable($conn, 'switchers');
$cnt_splitters = countTable($conn, 'splitters');
$cnt_ups       = countTable($conn, 'ups');
$cnt_others    = countTable($conn, 'others');
...
$inventory = [
    ...
    'Video Switchers' => $cnt_switchers,
    'Splitters'      => $cnt_splitters,
    'UPS'            => $cnt_ups,
    'Others'         => $cnt_others,
];
arsort($inventory);
...
/* ════════════════════════════════════════════
   ...
   ════════════════════════════════════════════ */
$acq_tables = ['laptops', 'desktops', 'printers', 'cameras', 'headsets', 'switches', 'routers', 'firewalls', 'switchers', 'splitters', 'ups', 'others'];
...
/* ── Friendly display labels for acquisition device names ── */
$acq_label_map = [
    'Switchers' => 'Video Switchers',
    'Ups'       => 'UPS',
];
$acq_per_device_display = [];
foreach ($acq_per_device as $key => $vals) {
    $label = $acq_label_map[$key] ?? $key;
    $acq_per_device_display[$label] = $vals;
}
$acq_per_device = $acq_per_device_display;
...
$j_dev_switchers = json_encode($dev_switchers);
$j_dev_splitters = json_encode($dev_splitters);
$j_dev_ups       = json_encode($dev_ups);
$j_dev_others    = json_encode($dev_others);
...
<body>
    ...
    <script>
        ...
        const DEV_SWITCHERS = <?= $j_dev_switchers ?>;
        const DEV_SPLITTERS = <?= $j_dev_splitters ?>;
        const DEV_UPS = <?= $j_dev_ups ?>;
        const DEV_OTHERS = <?= $j_dev_others ?>;
        ...
        const COLOR_MAP = {
            ...
            'Routers': '#ec4899',
            'Firewalls': '#ef4444',
            'Video Switchers': '#0ea5e9',
            'Splitters': '#a855f7',
            'UPS': '#eab308',
            'Others': '#9ba3b8'
        };
        const OTHER_COLORS = OTHER_LABELS.map(l => COLOR_MAP[l] || '#9ba3b8');
