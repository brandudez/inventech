...
if ($_SESSION['user']['role_id'] != 2) { header("Location: ../../index.php"); exit(); }
...
if ($active_filter !== '') { $where[] = "h.is_active = ?"; $params[] = $active_filter; $types .= 'i'; }

$whereSQL = !empty($where) ? "WHERE " . implode(" AND ", $where) : "";
$baseJoin = "FROM headsets h LEFT JOIN personnels per ON h.personnel_id = per.id LEFT JOIN ranks r ON per.rank_id = r.id LEFT JOIN divisions d ON h.division_id = d.id";
...
$aw = $where; $aw[] = "h.is_active = 1"; $aSQL = "WHERE " . implode(" AND ", $aw);
...
$iw = $where; $iw[] = "h.is_active = 0"; $iSQL = "WHERE " . implode(" AND ", $iw);
...
$stmt = $conn->prepare("SELECT h.*, CONCAT(COALESCE(r.rank,''),' ',per.first_name,' ',per.middle_name,' ',per.last_name) AS fullname, d.division AS division_name $baseJoin $whereSQL ORDER BY h.id DESC LIMIT ?,?");
...
<div class="top-bar">
    ...
    <div class="right-side">
        ...
        <button type="button" class="btn add-btn" data-bs-toggle="modal" data-bs-target="#addHeadsetModal">Add Headset</button>
    </div>
</div>

<!-- ADD MODAL -->
<div class="modal fade" id="addHeadsetModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-scrollable modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header text-white" style="background-color:#0d6ea8;">
                <h5 class="modal-title">Add Headset</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body">
                <form action="add_headsets.php" method="POST">
                    <input type="hidden" name="save_headset" value="1">
                    <div class="row g-3">
                        <div class="col-md-6">
                        ...
                        <div class="col-md-12">
                            ...
                            <div class="dropdown w-100">
                                ...
                                <div class="dropdown-menu w-100 p-2" style="max-height:250px;overflow-y:auto;">
                                    <?php foreach ($allPersonnel as $p): $fn = trim(($p['rank'] ?? '') . ' ' . ($p['last_name'] ?? '') . ' ' . ($p['first_name'] ?? '') . ' ' . ($p['middle_name'] ?? '')); ?>
                                        <div class="form-check">
                                            <input class="form-check-input" type="checkbox" name="previous_handlers_id[]" value="<?= $p['id'] ?>" id="addHsPh<?= $p['id'] ?>">
                                            <label class="form-check-label" for="addHsPh<?= $p['id'] ?>"><?= htmlspecialchars($fn) ?></label>
                                        </div>
                                    <?php endforeach; ?>
                                </div>
                            </div>
                    ...
                    <div class="modal-footer mt-3">
                        ...
                        <button type="submit" class="btn text-white" style="background-color:#0d6ea8;">Save Headset</button>
                    </div>
                </form>
            </div>
        </div>
                    ...
                    <?php while ($row = $result->fetch_assoc()): ?>
                        ...
                        <div class="modal fade" id="editHsModal<?= $row['id'] ?>" tabindex="-1" aria-hidden="true">
                            <div class="modal-dialog modal-lg modal-dialog-scrollable modal-dialog-centered">
                                <div class="modal-content">
                                    <div class="modal-header text-white" style="background-color:#0d6ea8;">
                                        <h5 class="modal-title">Edit Headset</h5>
                                        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                                    </div>
                                    <div class="modal-body">
                                        <form action="edit_headsets.php" method="POST">
                                                    ...
                                                    <div class="dropdown w-100">
                                                        ...
                                                        <div class="dropdown-menu w-100 p-2" style="max-height:250px;overflow-y:auto;">
                                                            ...
                                                            foreach ($allPersonnel as $p): $fn = trim(($p['rank'] ?? '') . ' ' . ($p['last_name'] ?? '') . ' ' . ($p['first_name'] ?? '') . ' ' . ($p['middle_name'] ?? '')); ?>
                                                                <div class="form-check">
                                                                    <input class="form-check-input" type="checkbox" name="previous_handlers_id[]" value="<?= $p['id'] ?>" id="editHsPh<?= $row['id'] ?>_<?= $p['id'] ?>" <?= in_array($p['id'], $selH) ? 'checked' : '' ?>>
                                                                    <label class="form-check-label" for="editHsPh<?= $row['id'] ?>_<?= $p['id'] ?>"><?= htmlspecialchars($fn) ?></label>
                                                                </div>
                                                            <?php endforeach; ?>
                                                        </div>
                                                    </div>
