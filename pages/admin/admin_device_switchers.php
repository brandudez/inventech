...
<body>
    ...
    <div class="top-bar">
        <div class="search-container">
            <form method="GET" action="admin_device_switchers.php" class="search-form">
        ...
        <div class="right-side">
            <div class="filters">
                <form method="GET" action="admin_device_switchers.php" id="filterForm">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="add_switchers.php" method="POST">
                        <input type="hidden" name="save_switcher" value="1">
                        <div class="row g-3">
                            <div class="col-md-6">
                                ...
                                <div class="dropdown w-100">
                                    ...
                                    <div class="dropdown-menu w-100 p-2" style="max-height:250px;overflow-y:auto;">
                                        <?php foreach ($allPersonnel as $p):
                                            $fn = trim(($p['rank'] ?? '') . ' ' . ($p['last_name'] ?? '') . ' ' . ($p['first_name'] ?? '') . ' ' . ($p['middle_name'] ?? '')); ?>
                                            <div class="form-check">
                                                <input class="form-check-input" type="checkbox" name="previous_owners_id[]" value="<?= $p['id'] ?>" id="addSwPh<?= $p['id'] ?>">
                                                <label class="form-check-label" for="addSwPh<?= $p['id'] ?>"><?= htmlspecialchars($fn) ?></label>
                                            </div>
                                        <?php endforeach; ?>
                                    </div>
                                </div>
        ...
        <div class="table-container">
            <table class="users-table">
                ...
                <tbody>
                                    ...
                                    <div class="modal-content">
                                        <div class="modal-header text-white" style="background-color:#0d6ea8;">
                                            <h5 class="modal-title">
                                                <i class="bi bi-diagram-3-fill me-2"></i>Switcher Details — <?= htmlspecialchars(($row['brand'] ?? '') . ' ' . ($row['model'] ?? '')) ?>
                                            </h5>
                                            <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                                        </div>
                                        <div class="modal-body">
                                        ...
                                        <div class="modal-footer">
                                            ...
                                            <button type="button" class="btn btn-primary"
                                                ...
                                                data-edit-target="#editSwitchersModal<?= $row['id'] ?>">
                                                                ...
                                                                foreach ($allPersonnel as $p):
                                                                    ...
                                                                    <div class="form-check">
                                                                        <input class="form-check-input" type="checkbox"
                                                                            ...
                                                                            value="<?= $p['id'] ?>"
                                                                            id="editSwPh<?= $row['id'] ?>_<?= $p['id'] ?>"
                                                                            <?= in_array($p['id'], $selHandlers) ? 'checked' : '' ?>>
                                                                        <label class="form-check-label" for="editSwPh<?= $row['id'] ?>_<?= $p['id'] ?>"><?= htmlspecialchars($fn) ?></label>
                                                                    </div>
                                                                <?php endforeach; ?>
                                                            </div>
                                                        </div>
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="14" class="text-center">No switchers found.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
